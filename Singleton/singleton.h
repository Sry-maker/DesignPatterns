...
protected:
	...
	std::string recordholder;  //记录保持者
	std::string record;        //记录
	Singleton1(const std::string name, const std::string holder, const std::string rec) : Olympicname(name), recordholder(holder), record(rec)//构造函数
	{
	}
	~Singleton1() {}//析构函数
...
public:
	...
	Singleton1(Singleton1& other) = delete;  //单例模式不可以进行的操作
	void operator=(const Singleton1&) = delete;

	static Singleton1* GetInstance(const std::string& name, const std::string& holder, const std::string& rec);//获取实例


	void recorddeclaration()   //记录打印
	{
		std::cout << "Singleton1::recorddeclaration : " << std::endl << "奥林匹克运动会项目: " << Olympicname << std::endl << "纪录保持者: " << recordholder << std::endl << "记录: " << record << std::endl;
	}
...
Singleton1* Singleton1::pinstance{ nullptr };
std::mutex Singleton1::mut;


Singleton1* Singleton1::GetInstance(const std::string& name, const std::string& holder, const std::string& rec)//获取实例
{
	std::lock_guard<std::mutex> lock(mut);
	if (pinstance == nullptr)//如果为nullptr则创建，否则直接返回已经创建好的
	{
		pinstance = new Singleton1(name, holder, rec);
	}
	return pinstance;
}
...
void singletoncreat()
{
	...
	std::cout << std::endl;
	Singleton1* singleton1 = Singleton1::GetInstance("男子200米", "博尔特", "19秒19");
	singleton1->recorddeclaration();
...
void singletonquery()
{
	...
	std::cout << std::endl;
	Singleton1* singl1 = Singleton1::GetInstance("", "", "");
	singl1->recorddeclaration();
